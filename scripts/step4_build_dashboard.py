...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  header { position: sticky; top: 0; z-index: 100;
    background: rgba(10,14,26,0.95); backdrop-filter: saturate(180%) blur(20px);
    -webkit-backdrop-filter: saturate(180%) blur(20px);
    border-bottom: 1px solid var(--border); }
  .container { max-width: 1440px; margin: 0 auto; padding: 14px 24px 8px; }
  .hero h1 { margin: 0; font-size: 19px; font-weight: 700; letter-spacing: -0.01em; }
  .hero .subtitle { color: var(--text-muted); font-size: 12.5px; margin-top: 3px; font-weight: 500; }
...
function makeDropdown({ label, options, multi, getSelected, onApply, withCount=false }) {
  ...
  function renderPanel() {
    ...
    options.forEach(opt => {
      ...
      const subLabel = opt.sub || '';
      const checked = multi ? temp.includes(v) : (temp === v);
      const item = document.createElement('div');
      item.className = 'dd-item' + (checked ? ' checked' : '');
      item.innerHTML = (multi ? '<span class="check"></span>' : '') + `<span>${escHTML(lbl)}</span>` + (subLabel ? `<span class="subs">${subLabel}</span>` : '');
      item.onclick = (e) => {
        e.stopPropagation();
        if (multi) {
...
function generateInsights(filtered) {
  ...
  if (allCreate.length) {
    ...
    const wkCreate = week.filter(v => (v.content_tags||[]).includes('创作'));
    if (wkCreate.length >= 3) {
      const wER = wkCreate.reduce((s,v) => s+(v.engagement_rate||0), 0) / wkCreate.length;
      const diff = ((wER - allER) / Math.max(allER, 0.01) * 100);
      if (Math.abs(diff) >= 15) insights.push(`创作类内容本周 ER ${diff > 0 ? '↑' : '↓'} ${Math.abs(diff).toFixed(0)}%（本周 ${wER.toFixed(2)}% vs 总体 ${allER.toFixed(2)}%）`);
    }
  }
  ...
  if (ipadAcc.length === 0) insights.push('当前筛选下无 iPad 配件相关视频，建议放宽时间或分组');
  else if (ipadAcc.length <= 5) insights.push(`iPad 配件相关视频仅 ${ipadAcc.length} 条，类目供给量低（蓝海机会）`);
  const kolWeek = week.filter(v => CFG.top_kol.includes(v.channel_name));
  if (kolWeek.length) insights.push(`头部 KOL 本周共发布 ${kolWeek.length} 条视频`);
  return insights;
}
...
function renderInsight(filtered) {
  ...
  if (insights.length === 0 && (!HISTORY || HISTORY.count >= 7)) { root.style.display = 'none'; return; }
  root.style.display = 'block';
  if (HISTORY && HISTORY.count < 7) {
    root.innerHTML = `<div class="title">📊 智能洞察</div><ul><li style="color:var(--text-faint)">趋势对比将在 7 天数据积累后开启（当前 ${HISTORY.count}/7 天）。先看实时观察 ↓</li>${insights.map(i => '<li>' + escHTML(i) + '</li>').join('')}</ul>`;
  } else {
